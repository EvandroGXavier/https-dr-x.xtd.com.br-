-- Migration: Fix column names in fn_provisionar_nova_empresa
-- Issue: Function was trying to return id/uuid_id but table only has empresa_id
...
BEGIN
  ...
  -- Ajuste: considerar registros antigos (email) e novos (superadmin_id)
  SELECT EXISTS (
    SELECT 1
    FROM public.saas_superadmins sa
    WHERE (sa.superadmin_id = v_chamador_id)
       OR (sa.email IS NOT NULL AND sa.email = auth.email())
  ...
  -- 2. CRIAR EMPRESA (apenas empresa_id existe, não id/uuid_id)
  INSERT INTO public.saas_empresas (
    nome, cnpj, ativa, plano
  ) VALUES (
    p_nome_empresa, v_cnpj_limpo, TRUE, 'Trial'
  ) RETURNING empresa_id INTO v_empresa_uuid;

  -- 3. CRIAR FILIAL MATRIZ (apenas filial_id existe, não id/uuid_id)
  INSERT INTO public.saas_filiais (
    empresa_id, nome, cnpj, matriz, ativa
  ) VALUES (
    v_empresa_uuid, 'Matriz', v_cnpj_limpo, TRUE, TRUE
  ) RETURNING filial_id INTO v_filial_uuid;

  -- 4. BUSCAR PLANO TRIAL (usar plano_id, não id)
  SELECT plano_id INTO v_plano_trial_id
