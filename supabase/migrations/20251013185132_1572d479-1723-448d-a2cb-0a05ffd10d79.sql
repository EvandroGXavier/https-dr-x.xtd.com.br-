-- Corrigir a RPC para casar com o payload do frontend e retornar JSONB
-- Reorganizar parâmetros: os obrigatórios primeiro, depois os com default
...
BEGIN
  ...
  -- 1) Criar contato base em contatos_v2
  INSERT INTO public.contatos_v2 (
    ...
    p_observacao,
    'juridica',
    COALESCE(NULLIF(p_classificacao, ''), 'cliente'),
    auth.uid(),
    auth.uid()
  ) RETURNING id INTO v_contato_id;

  -- 2) Gravar dados na tabela auxiliar contato_pj
  ...
  -- 3) Gravar endereços (array JSONB)
  IF jsonb_typeof(p_enderecos) = 'array' THEN
  ...
  -- 4) Gravar meios de contato (array JSONB)
  IF jsonb_typeof(p_meios_contato) = 'array' THEN
  ...
  PERFORM public.log_security_event(
    'contact_pj_created',
    format('Contato PJ criado: %s (CNPJ: %s)', v_nome_fantasia, v_cpf_cnpj),
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    PERFORM public.log_security_event(
      ...
      format('Erro ao criar contato PJ: %s', SQLERRM),
      jsonb_build_object('payload_keys', jsonb_object_keys(p_dados_pj), 'error', SQLERRM)
    );
    RAISE;
END;
