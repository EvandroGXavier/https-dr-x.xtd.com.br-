...
CREATE POLICY vinculos_select_policy ON public.contato_vinculos
  FOR SELECT USING (user_id = auth.uid() OR has_role('admin'::app_role));

CREATE POLICY vinculos_insert_policy ON public.contato_vinculos
  FOR INSERT WITH CHECK (user_id = auth.uid() OR has_role('admin'::app_role));

CREATE POLICY vinculos_update_policy ON public.contato_vinculos
  FOR UPDATE USING (user_id = auth.uid() OR has_role('admin'::app_role));

CREATE POLICY vinculos_delete_policy ON public.contato_vinculos
