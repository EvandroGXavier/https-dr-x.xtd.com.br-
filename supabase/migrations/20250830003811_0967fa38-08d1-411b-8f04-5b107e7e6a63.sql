...
USING (
  ...
  (has_role('moderator'::app_role) AND tenant_id = (SELECT tenant_id FROM profiles WHERE user_id = auth.uid()))
);
...
-- Função para obter tenant_id do usuário (placeholder - assumindo que existe na tabela profiles)
