...
BEGIN
  ...
  IF p_contato_id IS NULL THEN
    INSERT INTO contatos_v2 (
      nome_fantasia, cpf_cnpj, observacao, classificacao, tipo_pessoa, pessoa_tipo,
      empresa_id, filial_id, tenant_id, user_id, created_at, updated_at
    ) VALUES (
      p_nome, p_cpf_cnpj, p_observacao, p_classificacao, p_tipo_pessoa, p_pessoa_tipo,
      v_empresa_id, v_filial_id, v_user_id, v_user_id, NOW(), NOW()
  ...
  IF p_dados_pf IS NOT NULL AND p_tipo_pessoa = 'PF' THEN
    INSERT INTO contato_pf (
      contato_id, nome_completo, cpf, rg, data_nascimento, estado_civil, profissao,
      nacionalidade, naturalidade, mae, pai, empresa_id, filial_id, tenant_id, created_at, updated_at
    ) VALUES (
      v_contato_id, p_dados_pf->>'nome_completo', p_dados_pf->>'cpf', p_dados_pf->>'rg',
      (p_dados_pf->>'data_nascimento')::date, p_dados_pf->>'estado_civil', p_dados_pf->>'profissao',
      p_dados_pf->>'nacionalidade', p_dados_pf->>'naturalidade', p_dados_pf->>'mae', p_dados_pf->>'pai',
      v_empresa_id, v_filial_id, v_user_id, NOW(), NOW()
    )
    ON CONFLICT (contato_id) DO UPDATE SET
      nome_completo = EXCLUDED.nome_completo, cpf = EXCLUDED.cpf, rg = EXCLUDED.rg,
      data_nascimento = EXCLUDED.data_nascimento, estado_civil = EXCLUDED.estado_civil,
      profissao = EXCLUDED.profissao, nacionalidade = EXCLUDED.nacionalidade,
      naturalidade = EXCLUDED.naturalidade, mae = EXCLUDED.mae, pai = EXCLUDED.pai, updated_at = NOW();
  END IF;

  -- 3. UPSERT em contato_pj (se fornecido)
  IF p_dados_pj IS NOT NULL AND p_tipo_pessoa = 'PJ' THEN
    INSERT INTO contato_pj (
      contato_id, razao_social, nome_fantasia, cnpj, inscricao_estadual, inscricao_municipal,
      natureza_juridica, porte, data_abertura, atividade_principal, telefone_principal,
      email_principal, empresa_id, filial_id, tenant_id, created_at, updated_at
    ) VALUES (
      v_contato_id, p_dados_pj->>'razao_social', p_dados_pj->>'nome_fantasia', p_dados_pj->>'cnpj',
      p_dados_pj->>'inscricao_estadual', p_dados_pj->>'inscricao_municipal', p_dados_pj->>'natureza_juridica',
      p_dados_pj->>'porte', (p_dados_pj->>'data_abertura')::date, p_dados_pj->>'atividade_principal',
      p_dados_pj->>'telefone_principal', p_dados_pj->>'email_principal',
      v_empresa_id, v_filial_id, v_user_id, NOW(), NOW()
    )
    ON CONFLICT (contato_id) DO UPDATE SET
      razao_social = EXCLUDED.razao_social, nome_fantasia = EXCLUDED.nome_fantasia,
      cnpj = EXCLUDED.cnpj, inscricao_estadual = EXCLUDED.inscricao_estadual,
      inscricao_municipal = EXCLUDED.inscricao_municipal, natureza_juridica = EXCLUDED.natureza_juridica,
      porte = EXCLUDED.porte, data_abertura = EXCLUDED.data_abertura,
      atividade_principal = EXCLUDED.atividade_principal, telefone_principal = EXCLUDED.telefone_principal,
      email_principal = EXCLUDED.email_principal, updated_at = NOW();
  END IF;
  ...
  LOOP
    INSERT INTO contato_meios_contato (contato_id, tipo, valor, principal, empresa_id, filial_id, tenant_id)
    VALUES (v_contato_id, v_meio->>'tipo', v_meio->>'valor', COALESCE((v_meio->>'principal')::boolean, false),
  ...
  LOOP
    INSERT INTO contato_enderecos (
      contato_id, tipo, logradouro, numero, complemento, bairro, cidade, uf, cep, principal,
      empresa_id, filial_id, tenant_id
    ) VALUES (
      v_contato_id, v_endereco->>'tipo', v_endereco->>'logradouro', v_endereco->>'numero',
      v_endereco->>'complemento', v_endereco->>'bairro', v_endereco->>'cidade', v_endereco->>'uf',
      v_endereco->>'cep', COALESCE((v_endereco->>'principal')::boolean, false),
      v_empresa_id, v_filial_id, v_user_id
    );
  END LOOP;
...
-- 3. Corrigir função get_my_tenant_id
